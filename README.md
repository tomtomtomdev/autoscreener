...
7. Data refreshes **on demand**: every screener tab and the Watchlist fetch live when first revealed and whenever you tap Refresh. There's no background scheduler and nothing is cached to disk — each fetch pulls fresh from Stockbit. The Watchlist's live fan-out across every screener is throttled (sequential, randomised 1000–1500 ms gap) so Stockbit never sees a parallel burst, and an "as of HH:mm" badge shows when the displayed rows landed.
...
v1 + fifteen screener tabs (Accumulating, Above MA20, Shift Today, Accum/Dist Positive, 1M / 6M / 3M Net Foreign Flow, Foreign Buy Streak ≥5, Fresh Foreign Buy, Frequency Spike, Volume Spike, Above 50MA, Above 200MA, Liquidity Floor, Intraday Liquidity) + composite Watchlist are all shipped and working end-to-end against the real `exodus.stockbit.com` backend. The two liquidity tabs act as veto gates: a stock missing from either gets a red "ILLIQUID" flag in the Watchlist regardless of bandar score. Sign-in (trusted + new-device MFA), pre-flight token refresh, four-call screener bootstrap (paywall check + increment + template-with-page-1 + POST pages 2+), and infinite-scroll pagination are all in place. The Watchlist fan-out is throttled (sequential, randomised 1000–1500 ms gap) and cancellation-tolerant so a tab switch mid-bootstrap doesn't surface as a user-visible error. All data refreshes **on demand** — fetched live on first reveal and on the Refresh button, with no background scheduler or on-disk cache. A **Markets** tab browses the composite/indices/sectors and shows live-priced **Commodities** + **USD/IDR** rows (`emitten/{symbol}/info`), each drilling into an OHLCV candlestick chart.
