...
6. Results render in a sortable `Table` (`No · Symbol · Name · <metric 1> · <metric 2>`; the second metric column is omitted for single-column screeners like Accum/Dist Positive and the three foreign-flow tabs). Scrolling to the last row auto-loads the next page; pagination stops when Stockbit returns an empty page, a partial page below `limit`, or `total` is reached. The **Liquidity Floor**, **Intraday Liquidity**, and **Watchlist** tabs add a search field that filters rows by ticker (case-insensitive); on the two paginated screener tabs, typing a code first loads all remaining pages so the match isn't hidden behind lazy pagination.
