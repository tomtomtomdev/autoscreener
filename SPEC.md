...
- `PaperTradingAutopilotTests` (§18.4) — the hands-free rebalance: a due run books trades + stamps theses + the day; a second same-day call is a no-op (no re-fetch, no trades); the next trading day runs again; `autoExecute=false` refreshes caches but books nothing; a failed picks fetch leaves the day due for retry
...
**Autopilot (`PaperTradingAutopilot`, §18.4).** After each full IDX sweep the `DataSweepCoordinator` calls an injected `postSweep` hook → the autopilot's `runIfDue(now:)`, guarded to **once per trading day** (persisted as `PaperPortfolioState.lastAutoRebalanceAt`, same-calendar-day check) so the 5–10 min cadence can't over-trade. A due run refreshes the verdicts (on the current book) then the picks, builds the same recommendation-driven plan, and — when `autoExecute` (default on) — books it. The day is stamped only after a full pass succeeds (a failed picks fetch leaves it due to retry next sweep). The autopilot also refreshes the shared caches, so a run freshens the Recommendations screen too. Disabled under fixtures/tests (the seed sweep leaves the paper book deterministic; UI tests drive the manual flow).
...
├── PaperTradingAutopilot.swift      // @MainActor @Observable, hands-free once/day rebalance (runIfDue/isDue): refresh verdicts+picks → plan → book; injected engine sources (offline-testable); autoExecute flag
...
Features/Screener/DataSweepCoordinator.swift // gained an injected `postSweep: PostSweep?` hook, run on the main actor after a full IDX sweep — the app wires it to the autopilot's once/day rebalance (nil under fixtures/tests).
```
...
### 18.4 Autopilot (hands-free, once per trading day)

`PaperTradingAutopilot` runs the book off the recommendations without a tap. It's owned by `AppDependencies` and triggered by the `DataSweepCoordinator.postSweep` hook **after a full IDX sweep only** (fresh prices + regime; skipped on closed-session sweeps so it never trades on stale data). `runIfDue(now:)`:

1. **Guard** — `isDue(now:)` is false if `state.lastAutoRebalanceAt` is the same calendar day as `now` (so one auto-rebalance per trading day, regardless of the 5–10 min sweep cadence).
2. **Refresh** — pull the Gate-5 verdicts (reviewing the book *as it stands*) then the buy picks, writing both shared caches.
3. **Plan** — build candidates from the picks + `AllocationEngine.plan(...)` with the verdict overlay.
4. **Book** — when `autoExecute` (default on) and the plan has trades, `apply(plan:theses:)` (sells-then-buys, theses stamped). `autoExecute = false` refreshes the caches without trading.
5. **Stamp** — `recordAutoRebalance(at:)` persists the day **only after a full pass succeeds**; a failed picks fetch returns early un-stamped, so the next sweep retries.
