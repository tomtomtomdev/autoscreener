...
- [x] **Continuous market-hours sweep + disk cache** (§15): a single `DataSweepCoordinator` is the whole app's only fetch path. One throttled sweep (1–1.5 s between every request) fills two disk-backed stores — `ScreenerStore` (the 20 screeners) and `MarketDataStore` (Markets quotes + the regime read, §17). Open → full refresh every 5–10 min; closed → only the around-the-clock legs (global/commodity/FX) refresh every 20–30 min while the IDX-session legs stay frozen, except one forced full sweep just after the 16:00 close to capture the official closing figures (§15.2). The Watchlist and the Markets screen (and the now-unlisted per-screener views, still reachable in code) read from these caches — on a closed market or cold relaunch they render the last persisted snapshot with no network. A title-bar **Refresh** button (shown only when nothing is auto-refreshing — market closed, or open with continuous auto-fetch off) forces an immediate full sweep regardless of session; a ⌘, Settings **"Continuous auto-fetch"** toggle switches open-hours fetching between the continuous cadence and **boundary-only** (open/break/close) (§15.7).
- [x] **Recommendations** (unified): the buy-side picks and the Gate-5 sell-side review are merged into one ranked "Recommendations" inbox (EXIT→TRIM→BUY→HOLD, buys ordered by conviction; cards laid out two-up), the **default landing**. Supersedes the separate, hidden "Today's Picks" + "Positions to Review" screens. The composite Watchlist (the upstream radar) is rendered as a **section directly beneath the inbox on the same screen** (one shared scroll) — the two were merged into a single surface; "Watchlist" is no longer its own sidebar tab (§7.4/§15).
...
- [x] **Paper Trading — recommendation-driven 100M IDR portfolio** (§18): a persisted, mark-to-market paper account seeded with Rp 100,000,000 that runs off the **buy/sell recommendations** — it buys the Tier-A picks (§8) sized by the engine's `suggestedWeight`, sells on the Gate-5 verdicts, and gates total equity exposure by the market regime (§17) under explicit risk caps. Both a **manual** propose-then-confirm flow (generate a target rebalance with per-line rationale → review → Execute) and a **hands-free autopilot** (`PaperTradingAutopilot`: sell-side exits every warm sweep, buy-side rebalance once per session boundary) drive the same pure `AllocationEngine`; a disk-backed `PaperTradingStore` records confirmed fills. Holdings + realized/unrealized P&L track over time. No real orders.
...
`LoggingHTTPSession` also **mirrors failures to the unified `os.Logger`** (subsystem `com.tom.tom.tom.Autoscreener`, category `network`), so a 400 (and the exact feed/URL + redacted body that produced it) shows up in Xcode's console / Console.app without opening this panel. The line is built by the pure, testable `consoleLine(method:url:status:detail:)`: it returns `nil` for 2xx (success is never logged — keeps the noise low) and `⚠️ HTTP <code> <method> <url> — <detail>` otherwise, where `<code>` is the status or `transport-error` for a `URLError`, and `<detail>` (response body on an HTTP failure, error text on a transport failure) is truncated to 500 chars. Covered by `NetworkLogTests`.

### 6.4 Data section
A "Data" section with the **Continuous auto-fetch** toggle (id `settings.continuousAutoFetch`), bound to `SweepSettings.continuousAutoFetch` (§15.7). Caption: on → refresh every 5–10 min while open; off → only at open / break / close, with the toolbar **Refresh** for on-demand pulls.
...
| `CompanyPriceFeedError` (auth/paywall/network/decoding on `company-price-feed/historical/summary`) | Propagates out of the engine (a price-feed failure is global, not one un-valuable name) and fills the Recommendations screen as a readable sentence via `LocalizedError` — e.g. "Stockbit session expired — sign in again to load price data." (never the default "…CompanyPriceFeedError error 0") |

---

## 10. Testing

855 unit tests at present (`xcodebuild -only-testing:AutoscreenerTests test`). Coverage:
...
- `AllocationEngineTests` (§18) — the regime-gated allocator over `[AllocationCandidate]`: risk-off parks ≥70% in cash; risk-on deploys >60% but ≤95% (survive-first cash floor); nil regime → neutral band; no name exceeds the per-name cap; full deployment honours the position-count floor; **sizing** is proportional to `suggestedWeight` (independent of conviction/rank), falls back to fractional-Kelly conviction when `suggestedWeight == 0`, and fractional-Kelly damps the top name vs raw-proportional under `.conviction`; lot rounding to 100; empty/no-price/dropped-name handling; sub-band deltas suppressed. **Gate-5 exit overlay:** `.exit` fully sells a still-high-conviction held name + bars re-entry + overrides the anti-churn band; `.trim` caps at current (no add) but still rebalances down; `.hold`/empty matches the regime-only baseline
- `PaperTradingStoreTests` (§18) — fresh store seeded with 100M; buys spend cash + open positions; empty plan is a no-op; sell books realized P&L matching `Portfolio.apply`; reset returns to seed; portfolio (incl. `lastAutoRebalanceAt`) round-trips to `paper-trading-cache.json`; corrupt file keeps the seed
- `PaperTradingViewModelTests` (§18) — the screen's wiring (headless stand-in for the XCUITest): joins the stores, `canPlan` once prices load, `generatePlan()` buys **only recommended names** (warming a cold cache from the injected source), `execute()` books them + stamps theses + spends cash, `reset()` returns to seed; **Gate-5**: `generatePlan()` bars re-entry / sells a held name flagged in `ExitDecisionsStore`. `PositionReviewViewModelTests` covers the writer side (a review feeds the store)
- `PaperTradingAutopilotTests` (§18.4) — the hands-free autopilot: a due run books trades + stamps theses + the boundary; a second call **within the same session boundary** is a no-op (no re-fetch, no trades); **crossing into the next boundary** (and the next day) re-runs; `autoExecute=false` refreshes caches but books nothing; a failed picks fetch — or a **cold no-candidate sweep** — leaves the boundary due for retry; and the **asymmetric exit pass** liquidates an `.exit` name on a mid-session sweep while no rebalance is due, leaving unflagged holdings alone
- `DataSweepCoordinatorTests` (§15) — the `postSweep` hook fires after a full IDX sweep and **not** on a closed-only sweep (so the autopilot never trades on stale data)
- `MarketClockTests` (§15.1/15.7) — `isOpen`/`nextOpen`/`mostRecentClose` across session/lunch/weekend/pre-open; **boundary helpers**: `sweepBoundaryMinutes` = {09:00,12:00,13:30,16:00}, `mostRecentBoundary`/`nextBoundary` at each edge + pre-open/weekend fallback, `isWithinTradingDay` half-open window incl. lunch
- `SweepSettingsTests` (§15.7) — `continuousAutoFetch` defaults on when unset, persists off / back-on across instances via injected `UserDefaults`
- `DataSweepCoordinatorTests` boundary-only mode (§15.7) — auto-fetch **off** fires a full sweep when a boundary is owed, otherwise **pauses everything** (screeners *and* around-the-clock legs) and sleeps to the next edge; closed-market behaviour unchanged regardless of the toggle; a boundary capture still runs the autopilot when open
- `FetchStatusTests` (§15.7) — `autoFetchPaused` resolves to `.autoFetchOff(next:)` (outranks "updated", below error/paywall); label shows the mode + optional next edge; `GlobalRefreshButton.isVisible` hidden only while open-and-continuous
- `PriceSeriesTrendTests` — `PriceSeries.isUp` (the stock-detail line/gradient colour): up when latest close ≥ `previousClose`, first-bar fallback when `previousClose == nil`, flat→up, single-bar, empty→up
...
- `ManualRefreshUITests` (§15.7) — with the clock pinned (`-UITestMarketClosed`/`-UITestMarketOpen`): the global `globalrefresh` button appears when the market is closed and is hidden during normal open-hours auto-fetch. `GlobalFetchStatusUITests` now launches `-UITestMarketOpen` so its "no per-screen Refresh" assertion stays deterministic
...
**Obs + UI — console-mirror failed requests; drop Watchlist header spinner (2026-06-15).** Two small changes. (1) `LoggingHTTPSession` now mirrors every non-2xx / transport failure to the unified `os.Logger` (`category: network`) via a new pure `consoleLine(method:url:status:detail:)`, so a **400** and the exact feed/URL + redacted body surface in Xcode's console without opening the in-app Network panel (§6.3). Success (2xx) is never logged; detail is capped at 500 chars; transport errors are labelled `transport-error`. Tests (TDD): new `NetworkLogTests` (4 — 200→no line, 400 surfaces status/method/url/body, transport-error labelled, long-detail truncated) — **TEST SUCCEEDED**. (2) Removed the `ProgressView` spinner + "Loading… x/y" message that sat beside the **Watchlist** title in `WatchlistSection.header` — the global fetch-status surface already covers in-flight state, so the per-section indicator was redundant chrome; title + caption subtitle remain. **Build green.**

**Fix — `CompanyPriceFeedError` rendered as "error 0" on Recommendations (2026-06-15).** An expired/un-entitled Stockbit session made the essential price-feed leg (`StockbitDataProvider.fetchSecurity` → `priceFeed.dailyBars`) throw `CompanyPriceFeedError.unauthorized`, which `StockSelectionEngine.run()` correctly **propagates** (an auth/paywall failure is global — every name fails — so surfacing it beats a silent "no picks"; only `AdapterError`/`noPriceData` are per-name *skips*). But `CompanyPriceFeedError` was a plain `Error`, so `TodaysPicksViewModel`'s `error.localizedDescription` fell back to Swift's default `<Type> error <caseIndex>` and filled the screen with the literal **"CompanyPriceFeedError error 0"** (case 0 = `.unauthorized`). Fix: conform `CompanyPriceFeedError` to `LocalizedError` with a readable, actionable `errorDescription` per case — the same precedent already applied to `SelectionFundamentals.AdapterError` and `SelectionProviderError`. **No behavior change** beyond the message; propagate-vs-skip contract untouched (`EngineSkipResilienceTests` still green). Tests (TDD, Regression-Test pattern): new `CompanyPriceFeedErrorMessageTests` (3 — no case renders as "error 0"/bare type name, every case is a non-empty sentence, `.network` carries its detail). **Full price-feed + `EngineSkipResilienceTests` + `RecommendationsViewModelTests`: TEST SUCCEEDED.**

**Fix — `GrahamValuator` valued cash-rich names *below* their liquidation floor (2026-06-16).** The industrial intrinsic value combined the earnings-based **Graham number** `√(22.5·EPS·BVPS)` and the asset-based **NCAV** `(currentAssets − totalLiabilities)/shares` with `min` (`StockSelectionEngine.swift`, `GrahamValuator.intrinsicValue`). That **inverts** the NCAV signal: grounded in the two listed Graham skills (consulted in-session, not priors) — `intelligent-investor` (the Graham number is a fair-value *ceiling*; NCAV is a *separate* enterprising-investor deep-value screen) and `graham-financial-statements` (NCAV is the "net-net" liquidation *floor* — a business is worth **at least** its net liquid assets) — intrinsic value must be the **greater** of the two. Under `min`, any name with a positive NCAV was valued below its own net-current-asset value, sinking the margin-of-safety gate hardest for the cash-rich, low-debt balance sheets it should favour (a profitable name with NCAV 500 < Graham 1,500 was valued at 500 ⇒ MoS −100% ⇒ wrongly screened out). Fix: `candidates.min()` → `candidates.max()` (one line + doc comment); a non-positive NCAV stays a non-candidate, so the common going-concern case is unchanged (IV = Graham number). **The `GrahamValueScorer` computes its own Graham number independently of the valuator, so composite scores, ranking, and position weights are untouched** — only the reported IV, MoS, and the gate's "MoS X vs req Y" audit line move. Tests (TDD, Regression-Test pattern): new `GrahamValuatorTests` (4 — positive-NCAV-below-Graham doesn't drag IV down [the bug, red→green]; positive-NCAV-above-Graham becomes the floor; non-positive NCAV leaves IV at the Graham number; MoS uses the corrected floor). Several **characterization fixtures were internally impossible** (`currentAssets 4,500B > totalAssets`, built only so `min` would pick the Graham number) — repaired to consistent balance sheets with negative NCAV, **behavior-preserving so the golden master stays byte-for-byte**; the one intentional golden-master change is the consistent, cash-rich `EndToEndAuditTrailTests` WIFI fixture, whose IV assertion was corrected 6,364 → 8,000 (NCAV floor now binds, as it should). **Full `AutoscreenerTests`: TEST SUCCEEDED.** This was bug #1 of a four-item gate-strictness audit (`docs/SELECTION-GATE-STRICTNESS-FIXES.md`); the other three — bug #4 (trough-cyclical EPS), #3 (negative-WC solvency), #2 (bank calibration) — are completed next.

**Selection-gate strictness — bugs #4, #3, #2 (2026-06-16).** Completed the gate-strictness audit started with the NCAV fix above; full per-item diagnosis/decision/test log in `docs/SELECTION-GATE-STRICTNESS-FIXES.md`. Each followed the mandated workflow (green baseline → red regression test → minimal grounded fix → green + golden master), grounded in the matching investing skill, not priors. **#4 — trough cyclicals auto-eliminated (`adba22b`).** `GrahamValuator` fed the Graham number a single trailing TTM EPS, so any name with a negative trailing year got intrinsic value 0, failed the MoS gate, and was dropped before scoring — wrongly killing normally-profitable cyclicals. `intelligent-investor` specifies the Graham number's earnings input as the **average of the last 3 years**. *Targeted* variant (user decision): new `grahamEPS(_:config:)` keeps TTM EPS while positive (every profitable name byte-for-byte unchanged, no audit-trail inconsistency with `GrahamValueScorer`) and falls back to the 3-yr average (net income ÷ shares) only when the trailing year is a loss/zero; a persistent loss-maker (negative average) is still excluded. New config `ValuationParams.normalizedEpsYears` (=3). Tests: `troughYear…` (NI [150,170,−20], TTM −20 ⇒ avg 100 ⇒ IV 1,500; was 0 — red→green) + 2 no-regression guards. **#3 — `SolvencyGate` current-ratio ≥ 1.0 false-fails negative-WC names (`2e7b8b9`).** Telco/utility/regulated and supplier-financed staples/retail run a current ratio < 1 by design yet are sound; they hard-failed before scoring. `graham-financial-statements` tolerates a sub-2.0 ratio "only for utilities/regulated firms" and says to flag where modern asset-light businesses depart from the 1930s thresholds. Fix (user: *exempt entirely*): new `Solvency.currentRatioExemptSectors` (normalized IDX-IC `infrastruktur`, `barang konsumen primer`, `barang konsumen non-primer`) skip the current-ratio test; the **D/E guard still gates leverage for every name**, so the exemption never blanket-passes an over-levered company. The plan's interest-coverage/quick-ratio alternative was infeasible — no interest-expense/EBIT/inventory fields in the engine model. Tests: Infrastruktur @ current 0.6 now passes (red→green); a non-exempt sector @ 0.6 still fails; an exempt sector @ D/E 3.0 still fails on leverage. **#2 — bank justified-P/B excludes quality banks (`f32f7cd`).** Validated against a live BBCA-session capture (`stockbit.com.har`, the engine's exact `keystats/ratio/v1` + `comparison/ratios` for BBCA/BBNI/BBRI/BMRI): under the placeholder β 1.1 (Ke 14.2%), **BBNI at 0.88× book / 12.6% ROE was scored ~8% *overvalued*** — a sub-book value bank flagged as expensive. Two grounded changes (`damodaran-valuation`; user decisions): (a) **β 1.1 → 1.0** — large IDX banks are low-beta deposit franchises (BBCA regression β ~0.9–1.0; the code already flagged 1.1 a placeholder); Ke → 13.5%, BBCA justified P/B 2.07 → 2.27, IV 4,343 → 4,778. (b) **a separate financial-archetype MoS floor** = regime floor × new `bank.mosFloorMultiplier` (0.5 ⇒ ~15% neutral) — a justified-P/B IV is a Damodaran fair-value estimate, not a Graham-number ceiling, and a stable deposit franchise rarely trades at the 30–45% net-net discounts the industrial floor is calibrated for. New `SelectionProfile.requiredMarginOfSafety(policy:config:)` keeps the core loop archetype-agnostic (industrials return the policy floor unchanged ⇒ byte-for-byte). The g-cap is kept (Damodaran terminal discipline); it only binds BBCA among these names, so it changes no buy decision. Tests: `subBookValueBankIsNotValuedAsOvervalued` (BBNI-shaped MoS −7.6% → +1% — red→green); `qualityBankBelowJustifiedValueClearsTheLowerBankFloor` (BMRI-shaped MoS ~18.5%, recommended under the 15% floor, asserted < 30% to prove the lower floor admits it — red→green). BBCA golden values updated intentionally (IV 4,343 → 4,778), like the WIFI change in #1. **Full `AutoscreenerTests`: TEST SUCCEEDED; industrial golden master byte-for-byte throughout.** All four commits on local `main` (unpushed). Optional follow-ups noted in the plan doc (truly-live `selectionRunner` audit, per-bank measured β, broader multi-bank captures).

**Live audit harness + price-feed `limit` regression FOUND (2026-06-16).** Ran the long-deferred "truly-LIVE audit" follow-up — `selectionRunner.run()` against the *authenticated* Stockbit feed (couldn't be done before: no headless entry point). Added `Features/Selection/SelectionAudit.swift`, a headless audit gated behind the `-RunSelectionAudit` launch argument (no-op on a normal launch / xctest host / `-UITestFixtures`; `AutoscreenerApp.init()` invokes it, `ContentView` suppresses the GUI sweep under it). It confirms a Keychain token, ranks a small curated universe (default `WIFI, BBCA, BBNI, BMRI`; override `-SelectionAuditTickers A,B,C`) with the live buy engine, Gate-5-reviews the live paper book, prints a structured report, then exits; a `-SelectionAuditProbe` sub-mode exercises the price-feed endpoint directly to characterise its contract. Run it by exec'ing the built binary so stdout stays attached: `.../Autoscreener.app/Contents/MacOS/Autoscreener -RunSelectionAudit`. **Finding:** auth + `emitten/info` + `keystats` + `fundachart` all succeed live, but the buy run aborts with `400 INVALID_PARAMETER` from `GET company-price-feed/historical/summary/{SYM}` — and because that error is neither `AdapterError` nor `noPriceData`, the engine's per-ticker `catch` lets it **propagate and abort the whole run** (one bad first ticker empties the picks). Root cause, confirmed against `~/Downloads/stockbit.com.har` (the live web client's request) and an empirical variant probe: the server now **caps `limit` at 50** (50 OK, 60 ⇒ 400), but `CompanyPriceFeedService.dailyBars` requests `pageLimit: 1000`. Identical date span (365d), `page`, and `period=HS_PERIOD_DAILY` are all accepted; pagination via `next_page` is mandatory to cover the ≥200-day lookback regardless of `limit`. **FIXED (2026-06-16, `fix(selection)` commit) — test-first per the bug-fix workflow:** `CompanyPriceFeedServicing.dailyBars`'s default `pageLimit` 1000 → **50** (the live-verified cap; same span/`page`/`period` accepted, lookback covered by `next_page` pagination — 50 × `maxPages` 10 = 500 bars ≥ `betaLookback` 252, so no other change). Regression test `dailyBarsRequestsAServerLegalPageLimit` (a protocol spy asserts `dailyBars` never requests `limit > 50`) went red→green; the `buildsPathAndQuery` example was refactored off the now-invalid 1000. **Full `AutoscreenerTests` unit target: TEST SUCCEEDED; golden master byte-for-byte** (UI runner stays env-blocked on this box — *signal kill before connection* — unrelated). Open: re-run the live audit end-to-end to confirm real picks now rank (the original goal), and consider whether the engine's per-ticker `catch` should also treat a per-symbol price-feed failure as a skip rather than abort the whole run.

**UI — Recommendation cards two-up, buys ordered by conviction (2026-06-17).** The action-inbox cards rendered as a single-column `VStack` sorted purely by urgency (EXIT→TRIM→BUY→HOLD, tiebreak ticker). Two changes, both view-layer-only. **(1) Sort:** `RecommendationsViewModel.merge` keeps urgency as the primary key (the screen's "exits lead" discipline is preserved — conviction lives only on `.buy(Recommendation)`, never on the sell-side `ExitDecision`), but within the BUY group ties now break by `conviction` **descending** (then ticker for stability), so the strongest candidates surface first. Verdicts (exit/trim/hold) keep their ticker tiebreak unchanged. **(2) Layout:** the card `ForEach` moved from a `VStack` into a `LazyVGrid` with a **width-driven 1-or-2 column** count (`gridColumns`: two `.flexible()` columns once ≥700pt wide, one below that) — `.adaptive` was avoided because it can't cap the column count on a maximized window. Width is read by a background `GeometryReader` + a `WidthKey` `PreferenceKey` so measuring never perturbs layout (the project's existing `GeometryReader` idiom). The summary / skipped-note / footnote stay full-width; `ActionRowView` is unchanged (already `maxWidth: .infinity`), so each card just fills its grid cell; row accessibility ids (`recommendations.row.<TICKER>`) are preserved. Row-major fill puts the highest-priority / highest-conviction card top-left. **No VM-state / store / engine / composer change** — the golden master and the merged data path stay byte-for-byte. Tests (TDD): +2 `RecommendationsViewModelTests` (`mergeOrdersBuysByConvictionDescendingThenTicker` — conviction beats ticker within buys; `verdictsStillLeadBuysRegardlessOfConviction` — a 0.99-conviction buy never jumps an exit/trim, hold still last); the `rec(...)` object mother gained a `conviction:` arg. **Full `RecommendationsViewModelTests`: 10/10 TEST SUCCEEDED; `RecommendationsUITests` build + pass (skipped on this multi-display box via the standing `NSScreen.screens.count > 1` guard; run on single-display/CI).** Column count itself isn't XCUITest-assertable (the unit test proves ordering; the 2-col render + narrow→1-col collapse is visual). See §15.

**Paper Trading — driven by the buy/sell recommendations + hands-free autopilot (2026-06-17).** Paper trading was wired to *act on* the Recommendations screen instead of the raw composite Watchlist. Before: `AllocationEngine` built its buy universe from the entire Watchlist ranked by the bandar `WatchlistRow.score`, sizing by that score — it ignored the Tier-A engine, so a name that was never a BUY recommendation could still be bought, and size had nothing to do with conviction or `suggestedWeight`. Now: **(1) universe** — the allocator takes `[AllocationCandidate]` (flattened from the gate-filtered `Recommendation`s); a non-recommended name is never bought. **(2) sizing** — by the engine's own `suggestedWeight` (new `AllocationConfig.sizingBasis`, default `.suggestedWeight`), with fractional-Kelly conviction as the fallback and the existing per-name cap + water-fill + lot-rounding preserved; ranking is by conviction. **(3) execution** — a new `PaperTradingAutopilot` (§18.4) auto-rebalances **once per trading day** off a `DataSweepCoordinator.postSweep` hook (fired after a full IDX sweep only), guarded by `PaperPortfolioState.lastAutoRebalanceAt`; `autoExecute` defaults on. The manual Generate→Execute flow stays (now async, warming a cold recommendation cache from the same engine the Recommendations screen uses). Shared `PaperTradingPlanner` builds candidates + entry theses for both paths. New files: `PaperTradingPlanner.swift`, `PaperTradingAutopilot.swift`. The `postSweep` hook is trailing-defaulted (`nil`) so every existing coordinator caller is unchanged; wired off under fixtures/tests so the seed sweep leaves the paper book deterministic. Tests (TDD): migrated `AllocationEngineTests` to candidates + new `suggestedWeight` sizing/fallback cases; new `PaperTradingAutopilotTests` (once/day guard, next-day re-run, autoExecute-off, failed-fetch retry); rewrote `PaperTradingViewModelTests` for the async recommendation-driven flow; +2 `DataSweepCoordinatorTests` for the hook. **Full `AutoscreenerTests`: TEST SUCCEEDED** (incl. the `lastAutoRebalanceAt` persistence round-trip); app builds clean. `PaperTradingUITests` unchanged (BBCA is a clean BUY fixture) — skips on this multi-display box, runs on CI. See §18 / §18.4.

**Recommendations — rank from the sweep cache, not a live tab-open fan-out (2026-06-17).** The Recommendations inbox sat on "Sizing today's actions…" for minutes (effectively forever on a large watchlist): `todaysPicks`/`reviewPositions` built the live `StockbitDataProvider` and fetched `SecurityData` per name on tab open — ~13 throttled calls × the whole universe, serial, ~20–30 min. The per-symbol fan-out moved **onto the sweep** (§15.6). New `SecurityDataStore` (in-memory, per-ticker `SecurityData` + a `MarketContext`, freshness ≈ one trading day) is warmed by a new `DataSweepCoordinator.securitySweep` hook on every full IDX sweep — over **watchlist ∪ held positions**, before `postSweep` so the autopilot rebalances off a warm cache. A new read-only `CachedDataProvider` replaces the live provider in `todaysPicks`/`reviewPositions`, so the screen **never fetches**: a cache miss is a per-name `SelectionProviderError.notCached` **skip** (engine already skips `noPriceData`/`AdapterError`), and a cold cache (no context / no fresh candidate) short-circuits to `SelectionOutcome.awaitingData` → a **"Waiting for the data sweep…"** state (id `recommendations.awaitingData`) instead of a misleading "nothing to act on". The `SelectionAudit` CLI keeps the **live** `makeSelectionEngine`/`makePositionReviewer` (no sweep runs there). `securitySweep` is trailing-defaulted `nil` (existing coordinator callers unchanged) and wired off under fixtures/tests. Tests (TDD): new `SecurityDataStoreTests` (freshness/last-write-wins), `CachedDataProviderTests` (hit / `notCached` miss / no-context / engine-skips-an-empty-cache-instead-of-stalling), +2 `DataSweepCoordinatorTests` (`securitySweep` warms before `postSweep`; frozen on a closed sweep), +1 `TodaysPicksViewModelTests` (cold-cache `awaitingData`, stays unloaded to retry). **Full `AutoscreenerTests`: 0 failures; golden master byte-for-byte.** Deferred (cosmetic / follow-up): ~~the cache isn't persisted, so an after-hours cold launch shows "waiting" until the next IDX sweep (or a manual Refresh)~~ **[the persistence half is RESOLVED 2026-06-18 — a disk-backed `RecommendationsSnapshotStore` now restores the last displayed inbox on a cold launch; see the changelog entry and §15.6 below]**; and a cold tab retries on re-appearance, not live while you sit on it. See §15.6.

**Recommendations — rank the last close while the market's closed + capture the official 16:00 print (2026-06-17).** Two closed-market defects in the cache-backed inbox. **(1)** The `SecurityDataStore` freshness window is ~36h, but no warming sweep runs while closed — so once it aged out (Fri close + 36h ≈ Sun), the screen showed a perpetual **"Waiting for the data sweep…"** spinner over weekends/holidays even though the watchlist was populated and a good last-close snapshot existed (no sweep was coming until reopen). New pure `CacheReadPolicy(isOpen:)` decides the read window + label by session state: **open** → honour 36h (cold ⇒ honest "waiting", a warm sweep is imminent); **closed** → read the last-warmed close at *any* age and stamp `asOf` (`SecurityDataStore.lastWarmedAt()`), which `RecommendationsView` ranks under a **"Figures as of `<date>` · market closed"** caption (id `recommendations.asOfClose`). "Waiting" now shows only when open+cold or closed+never-warmed. **(2)** IDX's regular session ends 15:50, so the in-hours sweep never captured the **official 16:00 close**. The sweep loop now forces **one** full sweep after the close — `MarketClock.closingPrintMinute`/`mostRecentClose(asOf:)` + a `lastFullSweepAt` one-shot guard — warming data only via `runSweep(includeIDX:runAutopilot:)` (`runAutopilot: false`), so the once-a-day autopilot never trades off the forced sweep. Both `runAutopilot` and the new `lastFullSweepAt` init param are trailing-defaulted, so every existing coordinator caller is unchanged. Tests (TDD, failing-first): +6 `MarketClockTests` (`mostRecentClose` across session/post-close/weekend/pre-open), +3 `DataSweepCoordinatorTests` (closing capture warms-not-trades, skip-when-captured, one-shot) with two existing closed-loop tests updated for the new premise, +3 `SecurityDataStoreTests` (`lastWarmedAt` + unbounded read), new `CacheReadPolicyTests`, +1 each to `TodaysPicksViewModelTests`/`PositionReviewViewModelTests` (`asOf` surfaced). **Full `AutoscreenerTests`: TEST SUCCEEDED; characterization/golden-master byte-for-byte** (engine scoring untouched — only *which* cached data is read and *how* the closed state renders). See §15.1/§15.2/§15.6.

**UI — stock-detail price chart: candlestick → line + gradient fill (2026-06-18).** The Chart tab's price panel (`OHLCVChartView.priceChart`) was an OHLC candlestick (`RuleMark` wick + `RectangleMark` body, per-bar green/red). Replaced with a **close-price line + gradient area fill** (`LineMark` + `AreaMark` with a `LinearGradient` foreground, linear interpolation), coloured by the **whole-window** direction — green when the latest close is ≥ its reference (`previousClose`, else the first bar), red when below — via a new pure `PriceSeries.isUp`. The dashed previous-close `RuleMark`, the `.chartYScale(includesZero: false)` domain, and the `OHLCVPriceChart` accessibility id are unchanged; the **volume bars keep their per-bar up/down colour** (`color(for:)` retained; the candle-only `bodyWidth` helper removed). No data/service/VM/timeframe change — the line reads `close` from the same OHLCV candles, so `ChartService`/`OHLCVChartViewModel` tests are untouched. Tests (TDD, failing-first): new `PriceSeriesTrendTests` (6 — up/down/flat, `previousClose==nil` first-bar fallback, single-bar, empty). **Full `AutoscreenerTests`: 861 pass, 0 fail; golden master byte-for-byte** (no engine code touched). `StockDetailUITests` still asserts the chart via the unchanged `OHLCVPriceChart` id (CI proof; the UI runner is env-blocked on this box).

**Recommendations — show the last inbox on a cold launch instead of a spinner (2026-06-18).** Resolves the deferred persistence item from the §15.6 sweep-cache change above. The engine-input `SecurityDataStore` isn't persisted, so an after-hours cold launch (or the first visit before the sweep warms) showed "Sizing today's actions…" / "Waiting for the data sweep…" until the first IDX sweep landed. New disk-backed **`RecommendationsSnapshotStore`** (`Features/Selection/RecommendationsSnapshotStore.swift`; JSON at `…/Autoscreener/recommendations-cache.json`, same `DiskModel` + atomic-write + `loadFromDisk: !headless` idiom as `PaperTradingStore`/`MarketDataStore`, wired in `AppDependencies`) records the **last displayed inbox** — ranked picks + Gate-5 verdicts + skip note + `asOf`. `RecommendationsViewModel` restores it on init and renders it until live data arrives (**stale-while-revalidate**): `rows` returns the live merge if any, else `hasLoaded ? [] : cachedRows`, so an `awaitingData` pass or a failed refresh **keeps the cache** while a genuine *empty* success shows the real "nothing to act on today" (never stale); a full success persists the new inbox, and `asOf`/`skipped` fall back via an `isShowingCache` helper. Added a small non-blocking **"Updating…"** hint (id `recommendations.updating`) while a background refresh runs over a visible list. `Codable` added to `Recommendation`/`ExitDecision`/`ExitAction`/`SkippedName` (all trivially so). **Display-only and deliberately separate from the allocator-facing `RecommendationsStore`/`ExitDecisionsStore`** (those stay in-memory), so paper-trading behaviour and the golden master don't move. Gotcha: a `@MainActor` store init can't be a default-argument expression (nonisolated context) — construct it in the `@MainActor` body, mirroring the existing children `?? …` pattern; `Snapshot` is `Codable` but not `Equatable` (its members aren't). Tests (TDD): new `RecommendationsSnapshotStoreTests` (disk round-trip, `loadFromDisk` gate, corrupt-file tolerance, last-write-wins) + 7 cold-start cases in `RecommendationsViewModelTests` (cache shown before load; kept on awaiting/error; cleared on a genuine empty; replaced by live; persisted on success; not clobbered by an awaiting refresh). **Full `AutoscreenerTests`: TEST SUCCEEDED; golden master byte-for-byte.** UI runner env-blocked on this box — a cold-start XCUITest would need a seeded snapshot fixture (fixtures start empty); no fixture-only seam was added, so the unit suite is the proof. See §15.6.

**Paper Trading — autopilot rebalances per session boundary, exits every warm sweep (2026-06-19).** The hands-free autopilot was leaving the book stuck at 100% cash. Its once-per-**calendar-day** guard (`lastAutoRebalanceAt` same-day check) fired on the day's *first* sweep — the pre-open closing-capture — while the per-symbol cache was still cold, produced a no-trade plan, **stamped the day anyway**, then refused to run again until tomorrow; by the time the real picks warmed up on later sweeps the window was already closed (confirmed on disk: `trades:[]`, `lastAutoRebalanceAt` ≈ 08:21 pre-open, 100% cash, while the screener cache kept refreshing through 14:11). Three grounded fixes, test-first. **(1) Boundary scheduling** — `isDue(now:)` now keys off `MarketClock.mostRecentBoundary` (open 09:00 / break 12:00 / resume 13:30 / close 16:00) instead of the calendar day, so the book is re-evaluated at the open, after the lunch break, and on the official close (≈3 rebalances/day) rather than once; `PaperTradingAutopilot` gained an injected `MarketClock`. **(2) Cold-cache defer** — a no-candidate plan (cache still warming) no longer consumes the boundary: like a failed picks fetch it returns **un-stamped** to retry next sweep, so a cold first sweep can't strand the book in cash. **(3) Asymmetric buy/sell** (Marks: defense before offense; Zweig: cut losers fast) — exits no longer share the patient buy cadence. New `AllocationEngine.exitPlan` (sell-only — liquidates just the Gate-5 `.exit` names, leaves every other holding alone) runs **every warm sweep** via a new `runExits`; buys + regime `.trim`s stay **boundary-gated** in `runIfDue`; the `postSweep` hook now calls `run(now:)` = exits-then-(boundary)rebalance. Running `.exit` every sweep is safe — `ExitEvaluator` fires it only on fundamentals (failed hard gate / governance veto / collapsed intrinsic value / Graham overvaluation past a *negative*-margin hysteresis band), never on a drawdown or a daily price wiggle. Tests (TDD): `PaperTradingAutopilotTests` reworked for boundaries (same-boundary no-op, cross-boundary re-run, next-day, cold-defer regression) + 2 asymmetry tests (an `.exit` liquidates on a mid-session sweep while `!isDue`; an unflagged holding is untouched). **Full `AutoscreenerTests`: TEST SUCCEEDED.** See §18.2 / §18.4.

---

## 15. Continuous market-hours sweep + disk cache

There is **one fetch path** for the whole app: a single `DataSweepCoordinator` that fills two disk-backed stores — `ScreenerStore` (the 20 bandar screeners) and `MarketDataStore` (the Markets price quotes + the synthesised regime read, §17) — plus an in-memory `SecurityDataStore` (the per-symbol data the Recommendations engine ranks from, §15.6). Every screener tab, the composite Watchlist, the Markets screen, and the Recommendations inbox are thin read-only projections over those stores — they never fetch themselves. The two disk stores survive app restarts, so a relaunch (even with the market closed) renders the last captured snapshot immediately. Crucially, **every** outgoing Stockbit request — screener page, market quote, regime input, or per-symbol selection leg — passes through the **same anti-burst throttle**, so the app issues one request at a time and Stockbit never sees a parallel burst.
...
DataSweepCoordinator (@MainActor, @Observable)   ← one throttle for the whole sweep
   └─ runSweep(includeIDX:)
        ...
        ├─ securitySweep   (IDX-session) → SecurityDataStore (per-symbol SecurityData + MarketContext, §15.6)
        └─ postSweep       (IDX-session) → paper-trading autopilot (§18.4; runs after the cache is warm)
   └─ open → sleep 5–10 min;  closed → sleep 20–30 min;  repeat
ScreenerStore (disk-backed)         MarketDataStore (disk-backed)        SecurityDataStore (in-memory)
   ├─ ScreenerViewModel (per tab)     ├─ MarketQuotesViewModel → quotes    └─ CachedDataProvider →
   └─ WatchlistViewModel (composite)  └─ RegimeViewModel       → regime         StockSelectionEngine
```

### 15.1 Market clock (`Core/Market/MarketClock.swift`)

Pure value type. IDX regular sessions in **Asia/Jakarta, weekdays only**: Session 1 09:00–12:00 and Session 2 13:30–15:50 (half-open, so 15:50 sharp is closed). Fetching pauses over the 12:00–13:30 lunch break. `isOpen(at:)` and `nextOpen(after:)` take an injectable clock (tested at session boundaries / lunch / weekend). **Exchange holidays are not modelled** — a holiday weekday is treated as open (documented limitation; worst case a wasted sweep returning yesterday's values). `closingPrintMinute` (16:00) + `mostRecentClose(asOf:)` model the **official closing print**, which settles in the 15:50–16:00 closing auction — *after* the regular session ends — so the in-hours sweep (≤15:49) never sees it; the sweep loop uses these to capture each session's close once (§15.2). For the boundary-only mode (§15.7) it also exposes the **session edges** `sweepBoundaryMinutes` = {09:00 open, 12:00 break, 13:30 resume, 16:00 close} via `mostRecentBoundary(asOf:)` / `nextBoundary(after:)`, and `isWithinTradingDay(at:)` — the half-open `[09:00, 16:00)` weekday window *including* the lunch break (so the boundary loop stays active across 12:00–13:30; the 16:00 close hands back to the closed path).

### 15.2 Sweep loop & cadence

`start()` is idempotent (called from `ContentView` once signed in). The loop **always sweeps**, then sleeps a randomised gap: **open** → 5–10 min (full refresh); **closed** → 20–30 min (only the around-the-clock legs). `runSweep(includeIDX:runAutopilot:)` gates the **IDX-session legs** (the 20 screeners, composite/index/sector quotes, and the regime read) on `clock.isOpen()`; the **around-the-clock legs** (global indices, commodities, FX quotes — classified by `MarketGroup.isIDXSession`) run every sweep, since those instruments keep moving after the IDX close. When closed, the IDX legs are left frozen on their last snapshot. Each request is preceded by a randomised **1000–1500 ms** throttle (first request of the sweep free); the screener leg walks every page per screener up to a 20-page safety cap and writes each `ScreenerSnapshot{config, rows, fetchedAt}` **as it lands**. `refreshNow()` forces a full sweep (IDX legs included) regardless of session, wired to the global manual-Refresh button (§15.7). Mid-sweep cancellation is internal noise (partial snapshots kept, no error banner).

**Closing capture.** The regular session ends 15:50, so the in-hours sweeps never captured the **official 16:00 close** — leaving the selection cache (§15.6) on pre-close figures, then ageing it out entirely over a weekend/holiday. So while closed, if the last full sweep predates `clock.mostRecentClose(asOf:)` (tracked by `lastFullSweepAt`, set whenever the IDX legs run), the loop forces **one** full sweep to lock in the settled close. It's one-shot — the timestamp then sits past the close, so later closed ticks fall back to around-the-clock only. The forced sweep **also runs the autopilot** (`runAutopilot: open || capturingClose`): the 16:00 close is one of the autopilot's rebalance boundaries (§18.4), so the book rebalances off the official close and the every-sweep exit pass runs too (the autopilot's own boundary/`isDue` guard dedupes). (It also warms a cold cache on a weekend cold-launch, feeding the closed-market read path in §15.6.)
...
### 15.6 Per-symbol selection cache (`Features/Selection/SecurityDataStore.swift`)

The Tier-A `StockSelectionEngine` needs far more per name than the screener/market stores hold — annual financials, ~252 daily bars, daily foreign flow, free float, shares, broker/analyst/governance overlays (`SecurityData`, §8). The Recommendations screen used to assemble all of that **live on tab open** via `StockbitDataProvider`: ~13 throttled calls × the whole watchlist, serial, so the screen could sit on "Sizing today's actions…" for 20–30 min. That fan-out now moves onto the sweep.

- **`securitySweep` leg** — on every **full IDX sweep** (after prices + regime land, **before** `postSweep` so the autopilot rebalances off a warm cache), `AppDependencies.warmSecurityCache()` fetches each **watchlist ∪ held-position** name's `SecurityData` and one `MarketContext` once, through the shared throttle, and writes them to `SecurityDataStore` (in-memory, `[Ticker: (SecurityData, fetchedAt)]` + a cached context). Frozen on closed-only sweeps, like the other IDX legs; `refreshNow()` forces it after the close. Not persisted — `SecurityData` isn't `Codable`, and a full sweep refills it; a cold launch warms within the first sweep.
- **`CachedDataProvider`** (`DataProvider`, read-only) — `AppDependencies.todaysPicks` / `reviewPositions` now build the engine over a once-read `freshSnapshot(asOf:within:)` of the store instead of `StockbitDataProvider`, so the screen **never fetches** on tab open. A name not in the snapshot throws `SelectionProviderError.notCached`, which the engine treats as a per-name **skip** (never a live fetch); a missing/stale context throws `noRegimeInputs`. The freshness window is set by the pure `CacheReadPolicy(isOpen:)`: ~36h ("match the latest sweep") while **open**, unbounded while **closed** (see below).
- **Closed-market read** — while the market is **closed**, no warming sweep runs until the next session, so the 36h window used to strand the screen on "waiting" all weekend/holiday once the cache aged out. `CacheReadPolicy` instead reads the **last-warmed close regardless of age** when closed and stamps the outcome's `asOf` (= `SecurityDataStore.lastWarmedAt()`); `RecommendationsView` ranks those figures under a **"Figures as of `<date>` · market closed"** caption (id `recommendations.asOfClose`). Combined with the §15.2 closing capture, the cache holds the official close, so this is last-known-good, not stale-and-misleading.
- **Cold-cache UX** — when the cache has no context or not one fresh candidate, `todaysPicks`/`reviewPositions` short-circuit to `SelectionOutcome.awaitingData` (an empty/blocked watchlist is still a genuine "no picks", not "waiting"). `RecommendationsView` renders a **"Waiting for the data sweep…"** state (id `recommendations.awaitingData`) instead of a misleading "nothing to act on", and the child VMs leave `hasLoaded` false so a re-appearance retries. This now fires only when **open + cold** (a warm sweep is genuinely imminent) or **closed + never-warmed** (the closing-capture sweep is about to fill it) — not when a usable last-close snapshot exists, and on a **cold launch** the last *displayed* inbox is restored in its place (Cold-start display cache, below).
- **Cold-start display cache** (`Features/Selection/RecommendationsSnapshotStore.swift`) — the per-symbol engine-input cache above isn't persisted, so a cold launch (or the first visit before the sweep warms) would otherwise show the "Sizing…"/"Waiting…" spinner until the first sweep. A separate **disk-backed `RecommendationsSnapshotStore`** (JSON at `…/Autoscreener/recommendations-cache.json`, same `DiskModel` + atomic-write + `loadFromDisk: !headless` idiom as the other stores) records the **last successfully displayed inbox** — ranked picks + Gate-5 verdicts + skip note + `asOf`. `RecommendationsViewModel` restores it on init and renders it instead of a spinner until live data arrives (**stale-while-revalidate**): `rows` returns the live merge if non-empty, else `hasLoaded ? [] : cachedRows`, so an `awaitingData` pass or a failed refresh **keeps the cache** on screen while a genuine *empty* successful load shows the real "nothing to act on today" (never a stale list); a full success persists the new inbox (`asOf`/`skipped` follow via `isShowingCache`). A small non-blocking **"Updating…"** hint (id `recommendations.updating`) shows while a background refresh runs over the visible list. **Display-only and separate from the allocator-facing `RecommendationsStore`/`ExitDecisionsStore`** (those stay in-memory) — so paper-trading behaviour and the golden master are untouched. The engine-input `SecurityDataStore` stays un-persisted (it warms within the first sweep); this caches only the *rendered* output.
- **Live path retained** — the `SelectionAudit` CLI (a one-shot with no sweep) still builds the **live** `makeSelectionEngine` / `makePositionReviewer`, so its buy + Gate-5 output isn't starved by the empty cache.

### 15.7 Market-state fetch controls (`SweepSettings` + manual Refresh)

User control over fetch cadence, contextual to market state. Backed by `SweepSettings` (an `@Observable` `UserDefaults` wrapper, one flag `continuousAutoFetch`, default **on**; injected into the coordinator as an `@MainActor () -> Bool` accessor — same seam pattern as `now`/`sleeper` — and bound to the Settings toggle).

- **Continuous auto-fetch toggle (⌘, Settings → "Data", §6.4).** While the IDX is **open**: *on* (default) keeps the 5–10 min full-sweep cadence (§15.2, byte-for-byte unchanged); *off* puts the loop in **boundary-only** mode — inside the trading day (`isWithinTradingDay`) it fires a full sweep only when a session boundary (`needsBoundaryCapture`: `lastFullSweepAt < mostRecentBoundary`) has been crossed, otherwise **fetches nothing at all** and sleeps until the next edge (`nextBoundary`). So an *off* day refreshes at **open / break / resume**, with the 16:00 **close** still captured by the closing-capture path (§15.2) once `now` leaves the window. A boundary capture during a live session still runs the autopilot's per-boundary rebalance (`runAutopilot: clock.isOpen()`; the autopilot's own `isDue`/boundary guard dedupes), and the exit pass on every such warm capture. The setting governs **open hours only** — closed-market behaviour (around-the-clock legs + closing capture) is unchanged.
- **Manual Refresh button (`GlobalRefreshButton`, id `globalrefresh`).** A title-bar control beside the global status (§3-adjacent `.principal`/`.primaryAction` toolbar items on the shared `NavigationStack`), shown **only when nothing is auto-refreshing** — `isVisible = !(marketOpen && continuousAutoFetch)`, i.e. market closed **or** open-with-auto-off. Hidden during normal open-hours sweeping. Tapping it calls `refreshNow()` (full sweep → 20 screeners + watchlist + market + regime + the §15.6 recommendations cache); the Recommendations and Markets screens reload on sweep completion, so no per-screen wiring. Disabled while a sweep is in flight; a `TimelineView(.periodic)` re-evaluates visibility as the market opens/closes.
- **Status reflection.** When paused, `GlobalFetchStatusView` resolves to `FetchStatus.autoFetchOff(next:)` → **"Auto-fetch off · next `<HH:mm>`"** (outranks a stale "Updated", below errors/paywall).
- **Deterministic UI fixtures.** `-UITestMarketOpen` / `-UITestMarketClosed` pin `AppDependencies`' launch clock (`clockForLaunch`) to a fixed weekday-session / weekend instant so the market-state chrome is testable regardless of when the suite runs.
...
A persisted, mark-to-market paper account (seeded with **Rp 100,000,000**) that runs off the **buy/sell recommendations**: it buys the Tier-A picks (§8 — gate-filtered, conviction-ranked), sizes each by the engine's own `suggestedWeight`, sells on the Gate-5 exit/trim verdicts, and gates total equity exposure by the market regime (§17). It joins the signals the app already produces — the ranked `Recommendation`s + Gate-5 verdicts (`RecommendationsStore`/`ExitDecisionsStore`) and the regime read — and adds only an allocation + accounting layer. **It places no real orders; it is a simulation.** Both a **manual** propose-then-confirm flow and a **hands-free autopilot** (sell-side exits on every warm sweep; buy-side rebalance once per session boundary) drive the same allocator.

The screen reads four stores and writes one: `PaperTradingStore` (the portfolio), `MarketDataStore.regimeRead` (the regime), `RecommendationsStore`/`ExitDecisionsStore` (the buy/sell universe), and `ScreenerStore` (prices + display names). It never fetches directly — the unified sweep (§15) is the only network path; a cold recommendation cache is warmed once from the same headless engine the Recommendations screen uses.
...
2. **What to hold (the buy/sell recommendations).** The buy universe is the ranked Tier-A `Recommendation`s (gate-filtered picks, §8) — flattened to `AllocationCandidate {symbol, name, conviction, suggestedWeight}` (the engine depends on this DTO, not on `Recommendation`/`WatchlistRow`). Filter to priced + not Gate-5-`.exit`, rank by **conviction** desc, take the top **N** (default 12). A name that isn't a recommendation is never bought.
3. **How much of each (Against the Gods — `against-the-gods`).** Sized by `config.sizingBasis` (default `.suggestedWeight`): the raw per-name signal is the engine's own `suggestedWeight` verbatim (it already did the sizing); a name with no positive suggested weight falls back to fractional-Kelly conviction `convictionᵏ` (k = `kellyFraction`, default 0.5), and an all-zero vector degrades to an even split. The raw vector is normalised to the exposure, then **water-filled** under an effective per-name cap = `min(perNameCap, 1/minPositions)`. The cap both prevents one thesis from sinking the book and forces ≥ `minPositions` names once exposure is high enough (IDX names are highly correlated — correlations spike toward 1 in a crisis — so a count floor is the practical diversification lever). Targets are lot-rounded to 100 shares; deltas worth less than `rebalanceBandPct` × equity are suppressed (anti-churn).
...
### 18.2 Propose-then-confirm (manual) + autopilot (hands-free)

**Manual.** `generatePlan()` (async) warms the recommendation cache if cold (one load from the same engine the Recommendations screen uses), builds candidates via `PaperTradingPlanner.candidates(from:names:)`, then runs the pure `AllocationEngine.plan(state:candidates:regime:prices:exitDecisions:config:)` → an `AllocationPlan` of `AllocationLine`s (symbol, side, current/target/delta shares, price, est value, target weight, and a human **rationale** string — the same transparency ethos as `RegimeFactor.detail`). Nothing executes until the user clicks **Execute**: `PaperTradingStore.apply(plan:theses:)` books **sells before buys** (to free cash), pricing each fill through `ExecutionModel.standardIDX` (lot 100, buy 0.15%, sell 0.25%, slippage 0.05%), stamps an `EntryThesis` on each opening buy (Gate-5 Phase 3, via `PaperTradingPlanner.theses`), appends `PaperTrade`s, and persists. **Reset** returns the account to its 100M seed.

**Autopilot (`PaperTradingAutopilot`, §18.4).** After each full IDX sweep the `DataSweepCoordinator` calls an injected `postSweep` hook → the autopilot's `run(now:)`, which runs two passes on **asymmetric cadences** (Marks: defense before offense; Zweig: cut losers fast). **Defense — `runExits`, every warm sweep:** re-pull the Gate-5 verdicts on the current book and immediately liquidate any `.exit` name (broken thesis / failed gate / governance / price past IV) — capital protection can't wait for a boundary; it's sell-only (`AllocationEngine.exitPlan`), so an unflagged holding is left untouched. **Offense — `runIfDue`, once per session boundary:** guarded by `isDue(now:)` keyed off `MarketClock.mostRecentBoundary` (open / break / resume / close) so the 5–10 min cadence can't over-trade, it refreshes the picks, builds the recommendation-driven plan (buys + regime `.trim`s), and — when `autoExecute` (default on) — books it. A boundary is stamped (`PaperPortfolioState.lastAutoRebalanceAt`) only after a pass that traded or saw real candidates; a failed picks fetch **or** a no-candidate cold-cache sweep returns un-stamped to retry. The autopilot also refreshes the shared caches, so a run freshens the Recommendations screen too. Disabled under fixtures/tests (the seed sweep leaves the paper book deterministic; UI tests drive the manual flow).
...
├── Models/PaperTradingModels.swift  // PaperPosition, PaperTrade, PaperPortfolioState (seed/equity/P&L + avg-cost fill math mirroring Portfolio.apply + lastAutoRebalanceAt), AllocationCandidate, SizingBasis, AllocationConfig (.standard: Zweig bands + caps + Kelly + sizingBasis + score→exposure map), AllocationPlan/AllocationLine
├── AllocationEngine.swift           // pure enum (like RegimeSynthesizer): the 3-layer allocator over [AllocationCandidate], zero I/O
├── PaperTradingPlanner.swift        // pure glue: Recommendation[] → [AllocationCandidate] (candidates) + per-buy EntryThesis snapshots (theses); shared by the VM and the autopilot
├── PaperTradingStore.swift          // @MainActor @Observable, disk-backed (paper-trading-cache.json); apply(plan:theses:)/reset/recordAutoRebalance; version bump; same DiskModel/atomic-write/load-on-init pattern as MarketDataStore
├── PaperTradingViewModel.swift      // thin projection; equity/cash/P&L/holdings/trades/lastAutoRebalanceAt; async generatePlan (candidates from RecommendationsStore, warms a cold cache; ExitDecisionsStore overlay)/execute/reset
├── PaperTradingAutopilot.swift      // @MainActor @Observable, hands-free autopilot: run() = runExits (sell-only `.exit` liquidations, every warm sweep) + runIfDue (buys/trims, once per session boundary via isDue + MarketClock); injected engine sources (offline-testable); autoExecute flag
└── PaperTradingView.swift           // header (equity, cash, P&L, regime badge + target exposure, auto-rebalance status), proposed-plan table w/ Execute, holdings, trade log; accessibility ids (PaperTradingView, PaperTradingGenerateButton, PaperTradingExecuteButton, PaperTradingPlanRow_<SYM>, PaperTradingHoldingRow_<SYM>, PaperTradingAutoRebalanceStatus)

Features/Selection/ExitDecisionsStore.swift  // @MainActor @Observable, by-ticker → ExitAction cache (mirror of RecommendationsStore). Written by PositionReviewViewModel / the autopilot; read by the allocator. Lets it act on Gate-5 verdicts without re-running the expensive holdings review per rebalance.
Features/Screener/DataSweepCoordinator.swift // gained an injected `postSweep: PostSweep?` hook, run on the main actor after a full IDX sweep — the app wires it to the autopilot's `run(now:)` (every-sweep exits + per-boundary rebalance; nil under fixtures/tests).
```
...
### 18.4 Autopilot (hands-free — exits every sweep, rebalance per session boundary)

`PaperTradingAutopilot` runs the book off the recommendations without a tap. It's owned by `AppDependencies` and triggered by the `DataSweepCoordinator.postSweep` hook **after a full IDX sweep only** (fresh prices + regime; skipped on closed-session sweeps so it never trades on stale data). The hook calls `run(now:)`, which runs **two passes on asymmetric cadences** — defense fast, offense patient (Marks: defense before offense; Zweig: cut losers fast):

**`runExits(now:)` — every warm sweep, no boundary guard:**
1. **Review** — pull the Gate-5 verdicts on the current book.
2. **Liquidate** — `AllocationEngine.exitPlan(...)` builds a **sell-only** plan of just the `.exit`-flagged names (an unflagged holding is left alone — no rebalancing or trimming leaks into this cadence); when `autoExecute`, book it immediately. A broken thesis must not wait hours for the next boundary. Safe to run every sweep because `.exit` is fundamentals-driven (failed hard gate / governance veto / collapsed IV / Graham overvaluation past a *negative*-margin hysteresis band — never a drawdown or a daily price wiggle).

**`runIfDue(now:)` — once per session boundary (open / break / resume / close):**
1. **Guard** — `isDue(now:)` is true only when `state.lastAutoRebalanceAt` predates `MarketClock.mostRecentBoundary(asOf: now)`, so the book rebalances at the open, after the lunch break, and on the official close — not once a day, and not on every 5–10 min sweep.
2. **Refresh + Plan** — pull the buy picks (writing the shared caches) and build candidates + `AllocationEngine.plan(...)` with the verdict overlay (buys + regime `.trim`s).
3. **Book** — when `autoExecute` (default on) and the plan has trades, `apply(plan:theses:)` (sells-then-buys, theses stamped). `autoExecute = false` refreshes the caches without trading.
4. **Stamp** — `recordAutoRebalance(at:)` persists the boundary **only after a successful pass**; a failed picks fetch **or** a no-candidate cold-cache sweep (cache still warming) returns early un-stamped, so the next (warm) sweep retries instead of stranding the book in cash.

The manual Generate → Execute buttons remain available alongside the autopilot (e.g. to rebalance intraday). The screen surfaces the autopilot state via the `PaperTradingAutoRebalanceStatus` line. Engine sources are injected, so the whole pipeline is unit-tested offline; it's wired off (`postSweep == nil`) under fixtures/tests.
