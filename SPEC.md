...
- [x] **Recommendations** (unified): the buy-side picks and the Gate-5 sell-side review are merged into one ranked "Recommendations" inbox (EXIT→TRIM→BUY→HOLD), the **default landing**. Supersedes the separate, hidden "Today's Picks" + "Positions to Review" screens. The composite Watchlist remains the upstream radar.
