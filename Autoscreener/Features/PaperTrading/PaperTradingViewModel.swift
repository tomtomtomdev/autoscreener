...
final class PaperTradingViewModel {
    ...
    /// The fraction of equity this book targets deploying — the risk cap: the regime-aware exposure for
    /// the current score. The badge reads this so it stays truthful as the regime moves.
    var targetExposure: Double { config.exposure(forScore: regime?.score ?? 0) }
